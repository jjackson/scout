"""Payment data loader for CommCare Connect.

Fetches payment records from the v2 paginated JSON export endpoint
(``/export/opportunity/<id>/payment/``) and yields them unchanged —
...
class ConnectPaymentLoader(ConnectBaseLoader):
    ...
    def load_pages(self) -> Iterator[list[dict]]:
        total = 0
        for page in self._paginate_export_pages("payment/"):
            if not page:
                continue
            total += len(page)
            yield page
        logger.info("Fetched %d payments for opportunity %s", total, self.opportunity_id)
